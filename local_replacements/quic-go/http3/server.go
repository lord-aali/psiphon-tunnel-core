...
import (
	...
	psiphontls "github.com/Psiphon-Labs/psiphon-tls"
)
...
type Server struct {
	...
	// needed Port can be manually set when the Server is
	// created.
	//
...
func (s *Server) handleRequest(conn *connection, str quic.Stream, datagrams *datagrammer, decoder *qpack.Decoder) {
	...
	// [Psiphon]
	req.TLS = psiphontls.UnsafeFromConnectionState((*psiphontls.ConnectionState)(unsafe.Pointer(&cs)))
